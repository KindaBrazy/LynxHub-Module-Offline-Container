...
export function parseArgsToString(args: ChosenArgument[]): string {
  ...
  result += isEmpty(argResult) ? 'call launch-windows.bat' : `call launch-windows.bat ${argResult}`;
...
export function parseStringToArgs(args: string): ChosenArgument[] {
  ...
  lines.forEach((line: string): void => {
    if (!line.startsWith('call launch-windows.bat')) return;

    // Extract the command line arguments and clear falsy values
    const clArgs: string = line.split('call launch-windows.bat ')[1];
...
const stabilityRendererMethods: CardRendererMethods = {catchAddress, parseArgsToString, parseStringToArgs};

export default stabilityRendererMethods;
