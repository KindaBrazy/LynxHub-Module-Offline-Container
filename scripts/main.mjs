import { c as commonjsGlobal, i as isWin, p as parseArgsToString, a as parseStringToArgs, b as parseArgsToString$1, d as parseStringToArgs$1, e as parseArgsToString$2, f as parseStringToArgs$2, g as parseArgsToString$3, h as parseStringToArgs$3, j as parseArgsToString$4, k as parseStringToArgs$4, l as parseArgsToString$5, m as parseStringToArgs$5, n as getDefaultExportFromCjs, o as parseArgsToString$6, q as parseStringToArgs$6, r as parseArgsToString$7, s as parseStringToArgs$7, t as parseArgsToString$8, u as parseStringToArgs$8, C as ComfyUI_ID, A as AUTOMATIC1111_ID, L as LSHQQYTIGER_ID, v as LLLYASVIEL_ID, w as LSHQQYTIGER_FORGE_ID, V as VLADMANDIC_ID, M as MCMONKEYPROJECTS_ID, B as BMALTAIS_ID, O as OOBABOOGA_ID, R as RSXDALV_ID, G as GITMYLO_ID, S as SILLYTAVERN_ID, x as ANAPNOE_ID, y as ComfyUI_Zluda_ID, N as NEROGAR_ID, I as INVOKEAI_ID, E as EREW123_ID, z as OPEN_WEBUI_ID } from './RendererMethods_DBvoqT.mjs';
...
const LINE_ENDING$1 = isWin ? '\r' : '\n';
async function initBatchFile(path, data) {
    try {
        await gracefulFsExports.promises.access(path);
    }
    catch (error) {
        console.log(error);
        await gracefulFsExports.promises.writeFile(path, data, { encoding: 'utf-8' });
    }
}
async function utilRunCommands(batFileName, dir, defaultData) {
    if (dir && defaultData)
        await initBatchFile(path.join(dir, batFileName), defaultData);
    return `${isWin ? './' : 'bash ./'}${batFileName}${LINE_ENDING$1}`;
}
async function utilSaveArgs(cardDir, args, batFileName, parser) {
    const result = parser(args);
    const finalDir = path.join(cardDir, batFileName);
    await gracefulFsExports.promises.writeFile(finalDir, result);
}
async function utilReadArgs(cardDir, batFileName, defaultData, parser) {
    const finalDir = path.join(cardDir, batFileName);
    await initBatchFile(finalDir, defaultData);
    const data = await gracefulFsExports.promises.readFile(finalDir, 'utf-8');
    if (!data)
        return [];
    return parser(data);
}
...
async function getRunCommands$d(dir) {
    return await utilRunCommands(EXEC_FILE, dir, DEFAULT_BATCH_DATA$a);
}
async function saveArgs$8(cardDir, args) {
    return await utilSaveArgs(cardDir, args, CONFIG_FILE, parseArgsToString);
}
async function readArgs$8(cardDir) {
    return await utilReadArgs(cardDir, CONFIG_FILE, DEFAULT_BATCH_DATA$a, parseStringToArgs);
}
const a1MainMethods = { getRunCommands: getRunCommands$d, readArgs: readArgs$8, saveArgs: saveArgs$8 };
...
async function getRunCommands$c(dir) {
    return await utilRunCommands(BAT_FILE_NAME$b, dir, DEFAULT_BATCH_DATA$9);
}
async function saveArgs$7(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$b, parseArgsToString$1);
}
async function readArgs$7(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$b, DEFAULT_BATCH_DATA$9, parseStringToArgs$1);
}
const bmaltaisMainMethods = { getRunCommands: getRunCommands$c, readArgs: readArgs$7, saveArgs: saveArgs$7 };
...
async function saveArgs$6(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$a, parseArgsToString$2);
}
async function readArgs$6(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$a, DEFAULT_BATCH_DATA$8, parseStringToArgs$2);
}
const comfyMainMethods = { getRunCommands: getRunCommands$b, readArgs: readArgs$6, saveArgs: saveArgs$6 };
...
const erew123MainMethods = { getRunCommands: getRunCommands$a };
...
async function saveArgs$5(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$8, parseArgsToString$3);
}
async function readArgs$5(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$8, DEFAULT_BATCH_DATA$6, parseStringToArgs$3);
}
const gitmyloMainMethods = { getRunCommands: getRunCommands$9, readArgs: readArgs$5, saveArgs: saveArgs$5 };
...
const invokeMainMethods = { getRunCommands: getRunCommands$8 };
...
async function saveArgs$4(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$6, parseArgsToString$4);
}
async function readArgs$4(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$6, DEFAULT_BATCH_DATA$4, parseStringToArgs$4);
}
const mcMonkeyMainMethods = { getRunCommands: getRunCommands$7, readArgs: readArgs$4, saveArgs: saveArgs$4 };

const BAT_FILE_NAME$5 = isWin ? 'start-ui.bat' : 'start-ui.sh';
...
const nerogarMainMethods = { getRunCommands: getRunCommands$6, updateAvailable: updateAvailable$1 };
...
async function saveArgs$3(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$4, parseArgsToString$5);
}
async function readArgs$3(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$4, DEFAULT_BATCH_DATA$3, parseStringToArgs$5);
}
const oobaMainMethods = { getRunCommands: getRunCommands$5, readArgs: readArgs$3, saveArgs: saveArgs$3 };
...
async function getPipPackageVersion(packageName) {
...
const LINE_ENDING = platform$2() === 'win32' ? '\r' : '\n';
...
const openWebUIMainMethods = { getRunCommands: getRunCommands$4, updateAvailable, isInstalled, mainIpc };

const BAT_FILE_NAME$3 = 'lynx-user.bat';
const DEFAULT_BATCH_DATA$2 = '@echo off\n' +
    '\n' +
...
async function saveArgs$2(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$3, parseArgsToString$6);
}
async function readArgs$2(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$3, DEFAULT_BATCH_DATA$2, parseStringToArgs$6);
}
const comfyZludaMainMethods = { getRunCommands: getRunCommands$3, readArgs: readArgs$2, saveArgs: saveArgs$2 };

const BAT_FILE_NAME$2 = isWin ? 'start_tts_webui.bat' : 'start_tts_webui.sh';
async function getRunCommands$2(dir) {
    return await utilRunCommands(BAT_FILE_NAME$2, dir);
}
const rsxMainMethods = { getRunCommands: getRunCommands$2 };
...
async function saveArgs$1(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME$1, parseArgsToString$7);
}
async function readArgs$1(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME$1, DEFAULT_BATCH_DATA$1, parseStringToArgs$7);
}
...
async function saveArgs(cardDir, args) {
    return await utilSaveArgs(cardDir, args, BAT_FILE_NAME, parseArgsToString$8);
}
async function readArgs(cardDir) {
    return await utilReadArgs(cardDir, BAT_FILE_NAME, DEFAULT_BATCH_DATA, parseStringToArgs$8);
}
